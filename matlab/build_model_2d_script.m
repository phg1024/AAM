...
%	return;
    ...
    if isempty(shape_triangles)
        shape_triangles = delaunay(AAM.s0(np+1:2*np)', AAM.s0(1:np)');
    end
	
	% Prepare shape data for PCA
    % augment aligned data
    aligned_data_aug = aligned_data;
    aligned_data_aug(:, 1, :) = aligned_data_aug(:, 1, :);
    aligned_data_all = cat(3, aligned_data, aligned_data_aug);
    
	shape_matrix = reshape(aligned_data_all, [2*np ns*2]) - repmat(AAM.s0, [1 ns*2]);
	clear aligned_data;
	
    % Clean up the shape matrix
    addpath inexact_alm_rpca/
    addpath inexact_alm_rpca/PROPACK/

    shape_matrix_low_rank = inexact_alm_rpca(shape_matrix);  
...
%	return;
    ...
    mean_shape_verts = reshape(AAM.s0, [], 2);
    mean_mask_sum = zeros(modelh,modelw,3);
    for j=1:size(AAM.shape_mesh, 1)
        tri_verts_j = mean_shape_verts(AAM.shape_mesh(j,:), :);
        mean_mask{j} = repmat(poly2mask(tri_verts_j(:,2), tri_verts_j(:,1), modelh, modelw), [1, 1, 3]);
        mean_mask_sum = mean_mask_sum + mean_mask{j};
        %figure(3);imshow(double(mean_mask{j}));pause;
    end    
    %figure(3);imshow(double(mean_mask_sum));pause;
    AAM.mean_mask = mean_mask;
	
	app_matrix = zeros(modelw*modelh*size(app_data,3), ns);
	
	% For each image
	parfor i=1:ns    
		app_matrix(:,i) = reshape(pa_warp_m(AAM, shape_data(:,:,i), app_data(:,:,:,i)), [(modelw*modelh*size(app_data,3)) 1]);
    end
    ...
    %app_matrix_low_rank = app_matrix;
...
% 		
% 		subplot(1,2,1), imagesc(jacobian_i), axis equal; colormap jet;
% 		subplot(1,2,2), imagesc(jacobian_j), axis equal; colormap jet;
% 
% 		disp 'Waiting for keypress...'
% 		pause;
% 	end      
