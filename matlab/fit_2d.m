...
% Author: Luca Vezzaro (elvezzaro@gmail.com)
	...
	while iter <= max_it
		...
		warped_app = reshape(pa_warp_m(AAM, cur_shape, image_data), [], 1);
		
		% Error image, simply the difference between the image and A0
		error_img = warped_app - fitted_app;
		...
		if iter > max_it/2 || max_it < 10
            ...
            parfor step_i=1:length(steps)
                damping = steps(step_i);
                d_s0 = reshape(AAM.s0 - sum(AAM.s * damping * delta_qp(5:end), 2), np, 2);
                ...
                %d_s0 = d_s0 + rand(size(d_s0));

                comp_warps{step_i} = pa_warp_composition(AAM, cur_shape, d_s0);
                
                % Warp the appearance to compute the error image, as a column vector
                warped_app_i = reshape(pa_warp_m(AAM, comp_warps{step_i}, image_data), [], 1);

                % Error image, simply the difference between the image and A0
                error_img_i = warped_app_i - fitted_app;
                
                errors(step_i) = abs(sum(error_img_i.^2));
            end
            
            comp_warp_best = comp_warps{1};
            error_best = errors(1);            
            for i=1:length(steps)
            if errors(i) < error_best
                error_best = errors(i);
                comp_warp_best = comp_warps{i};
            end
            end
            
            comp_warp = comp_warp_best;
		else
...
imshow(image_data);
hold on;                                                                                       
triplot(double(AAM.shape_mesh), cur_shape(:,2), cur_shape(:,1), 'b');
triplot(double(AAM.shape_mesh), comp_warp(:,2), comp_warp(:,1), 'r');
subplot(1,2,2);
imshow([reshape(warped_app, modelh, modelw, []); reshape(fitted_app, modelh, modelw, []); reshape(error_img, modelh, modelw, [])]);
pause(0.1);
        ...
        if mod(iter, 5) == 0
            ...
            error_img = reshape(pa_warp_m(AAM, cur_shape, image_data), [], 1) - AAM.A0;

            damping = sqrt(iter / max_it);
            
            % Project the error image onto the appearance modes and use the resulting parameters
            % to build the reconstructed appearance
            fitted_app = (1-damping) * fitted_app + damping * (AAM.A0 + AAM.A * (AAM.A' * error_img));
        end
